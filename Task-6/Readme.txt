...
1. git stash - To save the unstagged and uncommitted work in present working directory.
2. git stash list - To list out the stored stash.
3. git stash pop - To restore the stored work.
4. git stash apply stash@{n} - To restore the work based on stash list order number(n).
5. git stash clear - To clear all the stash store.
6. git stash -u - To store the untracked file.
7. git stash drop stash@{n} - To drop the particular stored stash entry.